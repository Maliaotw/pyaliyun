...
from aliyunsdkslb.request.v20140515 import DescribeLoadBalancersRequest, DescribeLoadBalancerAttributeRequest, CreateLoadBalancerHTTPListenerRequest, \
    ...
    DeleteLoadBalancerRequest, SetLoadBalancerNameRequest, AddTagsRequest,TagResourcesRequest,ListTagResourcesRequest,DescribeTagsRequest
...
class SLB(AliClient):
    ...
    def addTagsRequest(self, id, k, v):
        request = AddTagsRequest.AddTagsRequest()
        request.set_LoadBalancerId(id)
        request.set_Tags([{'TagKey': k, 'TagValue': v}])
        response = self.do_action(request)
        return response


    def setInstanceName(self, id, name):
    ...
    def getInsList(self, detail=None):
        ...
        if not detail:
            ...
            self.ins_list = []
            for ins in  self._getInsList():
                ins.update(self.getDetail(ins['LoadBalancerId']))
